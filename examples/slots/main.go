...
var SymbolData = map[Symbol]SymbolInfo{
	SymbolDragon: {
		Name:      "Dragon",
		NameCN:    "龙",
		ImageFile: "slot_dragon_1766041758627.png",
		Payout:    map[int]int{3: 50, 4: 150, 5: 500},
	},
	SymbolPhoenix: {
		Name:      "Phoenix",
		NameCN:    "凤",
		ImageFile: "slot_phoenix_1766041775332.png",
		Payout:    map[int]int{3: 40, 4: 120, 5: 400},
	},
	SymbolTreasure: {
		Name:      "Treasure",
		NameCN:    "聚宝盆",
		ImageFile: "slot_treasure_pot_1766041794217.png",
		Payout:    map[int]int{3: 30, 4: 100, 5: 300},
	},
	SymbolWild: {
		Name:      "Wild",
		NameCN:    "百搭",
		ImageFile: "slot_wild_1766041810081.png",
		Payout:    map[int]int{3: 100, 4: 500, 5: 2000},
	},
	SymbolCardK: {
		Name:      "K",
		NameCN:    "K",
		ImageFile: "slot_card_k_1766041844584.png",
		Payout:    map[int]int{3: 10, 4: 30, 5: 100},
	},
	SymbolCardA: {
		Name:      "A",
		NameCN:    "A",
		ImageFile: "slot_card_a_1766041858660.png",
		Payout:    map[int]int{3: 10, 4: 30, 5: 100},
	},
	SymbolCardQ: {
		Name:      "Q",
		NameCN:    "Q",
		ImageFile: "slot_card_q_1766041875271.png",
		Payout:    map[int]int{3: 5, 4: 20, 5: 75},
	},
}
...
func (sm *SlotMachine) drawFrame(screen *ebiten.Image) {
	...
	vector.FillRect(
		...
		frameW+8,
		frameH+8,
		color.RGBA{R: 218, G: 165, B: 32, A: 255},
		false,
	)
	// Inner dark
	vector.FillRect(
		...
		frameW,
		frameH,
		color.RGBA{R: 15, G: 20, B: 45, A: 255},
		false,
	)
	...
	vector.FillRect(
		screen,
		float32(frameX)-titleW-5,
		float32(frameY)+frameH/2-titleH/2,
		titleW,
		titleH,
		color.RGBA{R: 139, G: 69, B: 19, A: 255},
		false,
	...
	vector.FillRect(
		screen,
		float32(frameX)-titleW,
		float32(frameY)+frameH/2-titleH/2+5,
		titleW-10,
		titleH-10,
		color.RGBA{R: 218, G: 165, B: 32, A: 255},
		false,
...
func (sm *SlotMachine) drawReels(screen *ebiten.Image) {
	for i, reel := range sm.Reels {
		...
		vector.FillRect(
			screen,
			float32(x),
			float32(sm.reelStartY),
			float32(reelWidth),
			float32(rowCount*symbolSize),
			color.RGBA{R: 25, G: 30, B: 55, A: 255},
			false,
		)
		...
		if reel.Spinning {
			...
			for j := -1; j <= rowCount+1; j++ {
				...
				if symY >= sm.reelStartY-symbolSize &&
					symY <= sm.reelStartY+float64(rowCount)*symbolSize+symbolSize {
					if symImg != nil {
		...
		vector.FillRect(
			screen,
			float32(x)-2,
			float32(sm.reelStartY)-symbolSize-10,
			float32(reelWidth)+4,
			float32(symbolSize)+12,
			color.RGBA{R: 15, G: 20, B: 45, A: 255},
			false,
		)
		vector.FillRect(
			screen,
			float32(x)-2,
			float32(sm.reelStartY)+float32(rowCount*symbolSize)-2,
			float32(reelWidth)+4,
			float32(symbolSize)+12,
			color.RGBA{R: 15, G: 20, B: 45, A: 255},
			false,
		...
		if i < reelCount-1 {
			...
			vector.FillRect(
				...
				2,
				float32(rowCount*symbolSize),
				color.RGBA{R: 218, G: 165, B: 32, A: 150},
				false,
			)
		}
	}
			...
			for _, row := range win.Rows {
				for col := 0; col < win.Count; col++ {
					...
					vector.StrokeRect(
						...
						float32(symbolSize),
						3,
						color.RGBA{R: 255, G: 215, B: 0, A: alpha},
						false,
					)
				}
			}
...
func (sm *SlotMachine) drawUI(screen *ebiten.Image) {
	...
	vector.FillRect(
		...
		float32(screenWidth),
		130,
		color.RGBA{R: 30, G: 20, B: 50, A: 255},
		false,
	)
	vector.FillRect(
		screen,
	...
	sm.drawInfoPanel(
		...
		int(panelY)+15,
		"总分",
		sm.formatNumber(sm.Credits),
		color.RGBA{R: 0, G: 255, B: 150, A: 255},
	)
	...
	vector.FillRect(screen, btnX, btnY+5, btnW, btnH, color.RGBA{R: 100, G: 30, B: 30, A: 255}, false)
	vector.FillRect(screen, btnX, btnY, btnW, btnH-5, btnColor, false)
	vector.FillRect(screen, btnX+5, btnY+5, btnW-10, btnH-15, btnInner, false)
	...
	// Auto-spin indicator
	if sm.AutoSpin {
		vector.FillCircle(screen, 620, panelY+55, 12, color.RGBA{R: 0, G: 255, B: 0, A: 255}, false)
...
func (sm *SlotMachine) drawInfoPanel(
	screen *ebiten.Image,
	x, y int,
	label, value string,
	...
	vector.FillRect(
		...
		panelW,
		panelH,
		color.RGBA{R: 40, G: 30, B: 60, A: 255},
		false,
	)
	vector.StrokeRect(
		screen,
	...
	vector.FillRect(
		...
		panelW-20,
		35,
		color.RGBA{R: 20, G: 15, B: 35, A: 255},
		false,
	)
